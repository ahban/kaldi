...
dir=exp/tri8b_debug2

global_cmvn_states=./global.ark
...
if [ $stage -le 4 ]; then
    ...
    relu-renorm-layer name=tdnn1 input=Append(-2,1) dim=256
...
if [ $stage -le 5 ]; then
    ...
    if $online_cmvn ; then
        if [ -z "$global_cmvn_states" -o ! -f "$global_cmvn_states" ]; then
            echo "$0: ban global cmvn states not specified for online cmvn"
            exit 1;
        fi
        mkdir -p $dir/egs
        cp $global_cmvn_states $dir/egs/global_cmvn.stats
    fi
    
    steps/nnet3/chain/aban-get-egs.py --stage=-10 \
        --cmd="$decode_cmd" \
